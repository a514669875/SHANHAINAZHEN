...
> **部署与存储（现行）**：交付默认为 **集中式**——在 **运行主后端的服务器**（封装一键启动默认端口 **18080**，开发时常用 **8000**）上保存 **SQLite 数据库** 与 **`backend/data`** 下的流程文件、归档与上传文件；用户通过局域网浏览器访问该服务器。下文若出现历史「分布式 / 各电脑存储」等表述，以实现与 [README](../README.md)、[deploy-packaged.md](../docs/deploy-packaged.md) 为准。
